...
export const useConnectionExplorerRouteController = () => {
  ...
  useEffect(() => {
    ...
    if (!draftIdParam) {
      setContext(null);
      setContextError("Missing draft ID. Open this route from site settings.");
      setContextLoading(false);
      return;
    }
  ...
  return {
    ...
    onBackToBuilder: () => {
      if (context?.draftId) {
        navigate(`/site-builder?draftId=${context.draftId}`);
        return;
      }
      if (draftIdParam) {
        navigate(`/site-builder?draftId=${draftIdParam}`);
        return;
      }
      navigate("/studio");
    }
